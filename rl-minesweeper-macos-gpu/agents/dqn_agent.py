...
def _mps_is_available() -> bool:
    """Return whether PyTorch MPS is usable on this machine."""
    mps = getattr(torch.backends, "mps", None)
    return bool(mps and mps.is_available() and mps.is_built())


def resolve_torch_device(device: str | None = None) -> torch.device:
    """Resolve the requested torch device, defaulting to the best available backend."""
...
class DQNAgent:
    ...
    def load_checkpoint(cls, path: str | Path, device: str | None = None) -> "DQNAgent":
        ...
        config["device"] = resolved_device.type
